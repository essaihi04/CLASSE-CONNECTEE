(function(){
  ...
  function normalizePlan(raw){
    ...
    plan.sessions=Array.isArray(plan.sessions)?plan.sessions:[];plan.sessions.forEach((session,si)=>{session.id=session.id||uid('session');session.title=String(session.title||`Séance ${si+1}`);session.durationMinutes=Math.max(1,Number(session.durationMinutes)||60);session.explanationMinutes=Math.max(0,Number(session.explanationMinutes)||Math.round(session.durationMinutes*.3));session.objective=String(session.objective||'');session.blocks=Array.isArray(session.blocks)?session.blocks:[];session.blocks.forEach(block=>{block.id=block.id||uid('block');block.type=BLOCK_TYPES[block.type]?block.type:'text';block.title=String(block.title||BLOCK_TYPES[block.type].label);block.durationMinutes=Math.max(1,Number(block.durationMinutes)||5);block.objective=String(block.objective||'');block.content=typeof block.content==='string'?block.content:JSON.stringify(block.content||'');block.say=typeof block.say==='string'?block.say:'';block.boardLines=Array.isArray(block.boardLines)?block.boardLines:null;block.resourceName=String(block.resourceName||'');block.presentation=block.presentation&&typeof block.presentation==='object'?block.presentation:null;block.activity=block.activity&&typeof block.activity==='object'?block.activity:null;block.simulation=block.simulation&&typeof block.simulation==='object'?block.simulation:null;block.simulationHtml=typeof block.simulationHtml==='string'?block.simulationHtml:'';block.image=block.image&&typeof block.image==='object'?block.image:null;block.evaluation=block.evaluation&&typeof block.evaluation==='object'?block.evaluation:null;block.teacherNote=String(block.teacherNote||'');block.validated=!!block.validated})});
    return plan;
  }
  ...
  function courseBlockRows(courseId,importId,sourceMap){
    const rows=[];
    state.plan.sessions.forEach((session,si)=>session.blocks.forEach((b,bi)=>rows.push({course_id:courseId,import_id:importId,session_position:si,position:bi,block_type:b.type,title:b.title,duration_minutes:b.durationMinutes,objective:b.objective,content:{text:b.content,say:b.say||'',board_lines:b.boardLines||null,session_title:session.title,session_duration_minutes:session.durationMinutes,explanation_minutes:session.explanationMinutes,presentation:b.presentation||null,activity:b.activity||null,simulation:b.simulation||null,simulation_html:b.simulationHtml||'',image:b.image||null,evaluation:b.evaluation||null},source_id:sourceMap[b.resourceName]||null,status:b.validated?'validated':'draft',teacher_notes:b.teacherNote||''})));
    return rows;
  }
  ...
  async function loadExistingCourse(sb,courseId){
    ...
    if(blocks&&blocks.length){
      ...
      plan.sessions=Object.keys(grouped).map(Number).sort((a,b)=>a-b).map((sessionIndex,newIndex)=>{
        ...
        return {id:template.id||'session-'+(newIndex+1),title:content.session_title||template.title||`Séance ${newIndex+1}`,durationMinutes:Number(content.session_duration_minutes)||Number(template.durationMinutes)||rows.reduce((n,row)=>n+Number(row.duration_minutes||0),0),explanationMinutes:Number(content.explanation_minutes)||Number(template.explanationMinutes)||0,objective:template.objective||'',blocks:rows.map(row=>({id:row.id,dbId:row.id,type:row.block_type,title:row.title,durationMinutes:Number(row.duration_minutes)||5,objective:row.objective||'',content:row.content&&row.content.text||'',say:row.content&&row.content.say||'',boardLines:row.content&&row.content.board_lines||null,presentation:row.content&&row.content.presentation||null,activity:row.content&&row.content.activity||null,simulation:row.content&&row.content.simulation||null,simulationHtml:row.content&&row.content.simulation_html||'',image:row.content&&row.content.image||null,evaluation:row.content&&row.content.evaluation||null,resourceName:sourceNames[row.source_id]||'',teacherNote:row.teacher_notes||'',validated:row.status==='validated'}))};
      });
    }
