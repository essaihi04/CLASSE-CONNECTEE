(function(){
  ...
  async function init(){
    try{await window.teacherAuthReady;const sb=window.classesSupabase,{data:{session}}=await sb.auth.getSession();let query=sb.from('courses').select('id,subject_id,grade_level_id,title,description,settings,subjects(code,name,color),grade_levels(code,name,sort_order),course_blocks(count)').eq('status','published');if(session&&session.user)query=query.eq('teacher_id',session.user.id);const {data,error}=await query.order('updated_at',{ascending:false});if(error)throw error;state.courses=data||[];
