...
:root{font-family:Inter,Arial,sans-serif;color:#12233f}*{box-sizing:border-box}html,body{height:100%;margin:0;overflow:hidden}body{background:#eef5ff;touch-action:manipulation}.app{display:flex;flex-direction:column;height:100%;background:#fff}.head{display:flex;align-items:center;gap:9px;padding:7px 12px;background:linear-gradient(120deg,#14376d,#2563eb);color:#fff}.head h1{font-size:clamp(15px,2.8vh,22px);margin:0;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.head button{border:0;border-radius:9px;padding:6px 11px;font-weight:800;cursor:pointer;background:#ffffff2b;color:#fff;font-size:13px;white-space:nowrap}.head button:hover{background:#ffffff45}.stage{position:relative;flex:1;min-height:0;padding:6px}.scene{position:relative;width:100%;height:100%;overflow:hidden;background:linear-gradient(#eef7ff,#fff);border:2px solid #bfdbfe;border-radius:14px;touch-action:none;user-select:none}.scene>img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;opacity:.16}.scene>svg{position:absolute;inset:0;width:100%;height:100%}.toast{position:absolute;left:50%;bottom:14px;transform:translateX(-50%);max-width:92%;background:#0f172ae0;color:#fff;padding:8px 15px;border-radius:12px;font-size:clamp(13px,2.2vh,17px);font-weight:700;opacity:0;transition:opacity .25s;pointer-events:none;text-align:center}.toast.show{opacity:1}.toast.ok{background:#14532de6}.toast.bad{background:#7f1d1de6}.controls{display:flex;gap:16px;padding:6px 12px;background:#f1f5f9;border-top:1px solid #dbe7f5}.controls[hidden]{display:none}.control{flex:1;display:flex;align-items:center;gap:8px;font-weight:800;font-size:13px;min-width:0}.control label{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.control output{color:#1d4ed8;min-width:52px;text-align:right}.control input{flex:1;height:32px;accent-color:#2563eb;min-width:60px}.element-label,.zone-label{font:800 3.8px Arial;fill:#102a56;text-anchor:middle;paint-order:stroke;stroke:#fff;stroke-width:.9px;stroke-linejoin:round;pointer-events:none}.zone-label{font-size:4.1px}.value-badge{font:800 3.6px Arial;fill:#1d4ed8;text-anchor:middle;paint-order:stroke;stroke:#fff;stroke-width:.9px}.draggable{cursor:grab;filter:drop-shadow(0 2px 2px #0f172a44)}.drag-hit{fill:transparent;stroke:none;pointer-events:all}.draggable:active{cursor:grabbing}.draggable:focus{outline:none;filter:drop-shadow(0 0 4px #f59e0b)}.zone-hit{stroke-width:1.2;stroke-dasharray:2 1;fill-opacity:.08}.zone-hit.active{fill-opacity:.22;stroke-width:2}
</style></head><body><main class="app"><header class="head"><h1>${escapeHtml(title)}</h1><button id="demo" type="button" title="Démonstration automatique">▶ Montrer</button><button id="reset" type="button" title="Recommencer">↺</button></header>
...
<script>'use strict';const DATA=${escapeJson(payload)};const S=DATA.spec;const variableState={},placements={},failures={};let timer=null,drag=null,toastTimer=null;
...
function speakWord(el){if(el&&el.word)voice('word',el.word,{elementId:el.id})}
...
function placeInZone(elementId,zoneId,announce,quiet){const el=S.elements.find(item=>item.id===elementId),zone=S.zones.find(item=>item.id===zoneId);if(!el||!zone||!zone.accepts.includes(elementId))return false;const accepted=zone.accepts.filter(id=>S.elements.some(item=>item.id===id)),slot=Math.max(0,accepted.indexOf(elementId)),cols=Math.max(1,Math.ceil(Math.sqrt(accepted.length))),rows=Math.max(1,Math.ceil(accepted.length/cols)),cellW=zone.width/cols,cellH=zone.height/rows,col=slot%cols,row=Math.floor(slot/cols);placements[elementId]={x:zone.x+col*cellW+(cellW-el.width)/2,y:zone.y+row*cellH+(cellH-el.height)/2,zoneId:zone.id};failures[elementId]=0;draw();const expected=S.elements.filter(item=>item.draggable&&S.zones.some(zone=>zone.accepts.includes(item.id))),done=expected.filter(item=>placements[item.id]&&placements[item.id].zoneId).length;if(announce){const complete=done===expected.length;showFeedback(complete?(S.successMessage||'Bravo, tout est bien placé !'):'Très bien ! Continue.',true,quiet?'':(complete?'success':'progress'),{elementId:elementId})}publishState();return true}
